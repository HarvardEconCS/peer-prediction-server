...
*/
	...
	private static void notifyWorkers(RequesterService service)
		...
		String subject = "Play a game with other turkers and earn up to $0.50 bonus";
	...
	private static void bonusManually(RequesterService service)
		...
		String query = "select hitId, assignmentId, payment, bonus from session "
				+ "where paid = 0 and bonusPaid = 0 and payment is null and bonus is not null and comment is not null";
		ResultSet rs = stmt.executeQuery(query);
		...
		while (rs.next()) {
			...
			if (assigns.length > 0 && bonusString != null) {
				...
				// "set payment=0.05, paid=1, bonusPaid=1, hitStatus='Disposed' "
				// +
				// "where hitId='%s' ", hitId);
